...
class BCTrainer:
    ...
    def collect_training_trajectories(
        ...
        # HINT1: depending on if it's the first iteration or not, decide whether to either
            ...
            # (3) If it is not the first iteration,
            # collect `self.params['batch_size']` transitions
        # HINT2: use sample_trajectories from utils (after the "Collecting data .." print statement).
        # HINT3: you want each of these collected rollouts to be of length self.params['ep_len']

        print("\nCollecting data to be used for training...")
        # load initial expert data with pkl file
        if itr == 0:
            try:
                ...
                # TODO: collect self.params['batch_size_initial'] transitions from collect_policy
                bsz = self.params['batch_size_initial']
                paths = utils.sample_n_trajectories(self.env, collect_policy, bsz, self.params['ep_len'], False)
                envsteps_this_batch = 0
                for path in paths:
                    envsteps_this_batch += utils.get_pathlength(path)
        else:
            # TODO: collect self.params['batch_size'] transitions
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        for idx in range(len(paths)):
            observation = paths[idx]['observation']
            paths[idx]['action']= expert_policy.get_action(observation)
